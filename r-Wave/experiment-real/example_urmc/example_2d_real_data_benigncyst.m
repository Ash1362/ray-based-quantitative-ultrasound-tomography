% Example for image reconstruction of the speed from the in-vivo BenignCyst
% transmission ultrasound dataset measured on a ring. The results obtained
...
% This in vivo dataset is freely available and should be downloaded from
% the link provided in the reference [5]. 

%%=========================================================================
% EXPERIMENTAL SETTING
%==========================================================================
% The transducers are placed on a ring with a radius of approximately 11cm. 
% For further details, See: Section ''IV. RESULTS, D. In-Vivo Demonstration''
% of reference [4].
%%=========================================================================
% IMAGE RECONSTRUCTION
%==========================================================================
% This script first loads the in-vivo dataset from the data_path, which is
% set 'data/experiment_rochester/' be default. The user must upload the
% data from the links provided in [5], and add the dataset: 'BenignCyst' to
% the data path before running this script.
...
% are chosen 4 in this script, i.e., the data for 128 emitters and
% 128 receivers are used for image reconstruction.
% However, the results in the associated paper [3] are based on setting both
% sampling rates 1, i.e., the data for all 512 emitters and 512 receivers
% were used for image reconstruction.
% The user's choice is between 1, 2 or 4. (A higher value leads to less
% computational cost, but lower accuracy, but the accuracy for choices 1 
% and 2 are almost the same.)
%%=========================================================================
% REFERENCES
%==========================================================================
...
% get the experiment number (always fixed for downloading the 'BenignCyst' 
% dataset)
experiment_number = 3;
...
% sampling rates 1, i.e., the data for all 512 emitters and 512 receivers
% were used for image reconstruction.
% The user's choice is between 1, 2 or 4. (Higher value leads to less
% computational cost, but lower accuracy, but the accuracy for settings 1 
% and 2 are almost the same.)

% get the downsampling rate for the emitters (1,2 or 4)
emitter_downsampling_rate = 4;

% get the downsampling rate for the receivers (1,2 or 4)
receiver_downsampling_rate = 4;
...
%==========================================================================
% get an estimated absorption coefficient [dB MHz^{-y} cm^{-1}] used for 
% only breast 
absorption_coeff_object = 0.5;

% get an estimated power-law absorption % Default: 1.1
% (my code does not accept power-law values less than 1.1)
...
sound_speed_time_window = 100; 


%%=========================================================================
% THE PARAMETERS WHICH ARE ALWAYS FIXED FOR ALL ROCHESTER DATASETS
%==========================================================================
...
reconst_args_tof = {'num_worker_pool', num_worker_pool,...
    ...
    'minimum_distance_coeff', minimum_distance_angular,...
...
if do_greens_approach
    ...
    % get an estimated alpha coefficient map [dB MHz^{-y} cm^{-1}]
    simulation_prop.alpha_coeff_real = zeros(size(img_tof));
    simulation_prop.alpha_coeff_real(img_tof > 1475) = absorption_coeff_object; 

    % get the exponent power
    simulation_prop.alpha_power = absorption_power; % Default = 1.1

    % display the abosorption coeffcient map
    figure; imagesc(simulation_prop.alpha_coeff_real); axis image; colorbar
    ...
    greens_args = {'num_worker_pool', num_worker_pool,...
        ...
        'num_frequency', 200,... % Default:200
        'num_frequency_level', 100,... % Default:100
        'num_frequency_per_level', 2,...
        'step_length', step_length_greens,...
        'f_range', [0.3e6, 1.25e6],...
        ...
        'cut_off', 0.5,...
        'order', 5,...
        'frequency_level', frequency_level:frequency_level + 9};
    ...
    for levels = 1:10
        ...
        [img_greens, out_greens, para_greens] = reconstructGreensImage(data,...
            ...
            img_greens, out_greens_tot.ray_initial_angles, simulation_prop,...
            greens_args{:});
        ...
        greens_args{end} = greens_args{end} + 10;
