...
%aux(setting.mask == 1 | setting.mask == 2) = 1500;
...
% aux = zeros(setting.Nxyz);   % coupling medium
% aux(setting.mask == 3) = 0;  % chest wall
% aux(phan == 1) = 0.6;  % fibro-glandular tissue
% aux(phan == 2) = 0.6;  % fat (0.75)
% aux(phan == 3) = 0.6;  % skin
% aux(phan == 4) = 0.6;  % blood vessel
% aux(phan == 5) = 0.75; % generic breast tissue
...
%if(add_absorption)
    ...
    % set breast tissue to uniform value
%    medium.alpha_coeff = zeros(setting.Nxyz);   % power law pre-factor
%    medium.alpha_coeff(phan > 0 | setting.mask == 3) = 0.75;
