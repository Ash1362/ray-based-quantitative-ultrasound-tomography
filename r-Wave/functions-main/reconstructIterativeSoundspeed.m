function [img, out, initial_angles] = reconstructIterativeSoundspeed(tof_discrepancy,...
...
if  recon_grid.dx < (5e-4)-(1e-10)
    error('The grid spacing smaller than 0.5mm is not necessary for a ray-based method.')
elseif  abs(recon_grid.dx - (5e-4))< 1e-10
    para.smoothing_window_size = 13;
elseif abs(recon_grid.dx - (7.5e-4))< 1e-10
    para.smoothing_window_size = 9;
elseif abs(recon_grid.dx - (1e-3))< 1e-10
    para.smoothing_window_size = 7;
elseif abs(recon_grid.dx - (2e-3))< 1e-10
    para.smoothing_window_size = 5;
elseif recon_grid.dx > (2e-3)+(1e-10)
...
if isfield(water, 'object_water')  && ~isstruct(receiver_positions)
    
    error(['If the struct water includes the field object_water,'...
        'receiver_positions must be a struct.'])
...
% remove the grid points which travel ouside the binary mask for
% ray tracing
system_matrix(:, ~mask_raytracing(:)') = [];

% get the binary vector for the rows which must be nulled
res_null = ~coeff_res_inv; %  | out.num_rays{1} > para.max_raylinking_iter-1;
...
for iterout = 1 : num_iterout
    ...
    % get the binary vector for the rows which must be nulled
    res_null = ~coeff_res_inv;   %  | out.num_rays{iterout} > para.max_raylinking_iter-1;
    ...
    if strcmp(para.linear_subproblem_method, 'sart')
        ...
        % calculate the inverse coefficients for the coumns (grid points)
        coeff_gridpoints_inv = sum(system_matrix, 1);
    ...
    if ~isempty(sound_speed_true)
        ...
        switch dim
            case 2
                ...
                % Calculate the relative error based on Eq. (76) in the
                % papaer:
                % A. Javaherian and B. Cox, ❝Ray-based inversion accounting for scattering
            ...
            case 3
                % Calculate the relative error based on Eq. (61) in the
                % papaer:
                % A. Javaherian, F. Lucka and B. T. Cox, ❝Refraction-corrected ray-based inversion for three-dimensional
                % ultrasound tomography of the breast❞, Inverse Problems, 36 125010.
                out.relative_error(iterout)= 100 * norm((img(mask_reconst)) - sound_speed_true(mask_reconst)).^2/...
                    norm(sound_speed_true(mask_reconst) - water.sound_speed_only_water).^2;
        end
