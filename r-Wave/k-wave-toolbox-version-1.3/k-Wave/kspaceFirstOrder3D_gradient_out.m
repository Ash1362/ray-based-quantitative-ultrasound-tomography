function [sensor_data, mem_usage] = kspaceFirstOrder3D_gradient_out(kgrid, medium, source, sensor, varargin) %#ok<STOUT>
...
for t_index = index_start:index_step:index_end
    ...
    if flags.use_sensor && ~flags.time_rev && (t_index >= sensor.record_start_index)
        ...
        % extract data from specified grid points
            ...
            % acoustic variables
            % p_gradient_z = real(ifftn(bsxfun(@times, permute(ifftshift(1i * kgrid.kz_vec), [2,3,1]), fftn(p)) ));
            p_gradient_z = p;
            %pg_z(kgrid.z_vec < -16.6e-3) = -pg_z(kgrid.z_vec < -16.6e-3);
            %sensor_data.p(:, file_index) = p(sensor_mask_index);
            sensor_data = kspaceFirstOrder_extractSensorData(3, sensor_data, file_index, sensor_mask_index, flags, record, p_gradient_z, ux_sgx, uy_sgy, uz_sgz);
