...
% In this example, the approach taken for ray tracing and computing the
% geomterical portion of the amplitudes for both Green's inversion approach
% is based on paraxial ray tracing. (See Algorithm 1 in [1])
...
% the paper [1]. This script will then compute the time-of-flights of the UST
% time series using an approach called Akaike-information-criterion. A
% sound speed image is then reconstructed from the time-of-flights using
% few iterations of a time-of-flight-based algorithm. Using the time-of-flight-based
% image reconstruction, each linearised subproblem is solved using 'SART'
% innter iterations. (cf. section 6.3.1 in [1].) For construction of the
% system matrix associated with each linearisation in the time-of-flight-based
% image reconstruction approach, the ray tracing can be done using the four algorithms
% analysed in the 'example_fish_eye.m' in this toolbox, but the
% 'Runge-kutta-2nd' was used in the papers [1] and [2], and this example.
% It is recommneded that 'Runge-kutta-2nd' is used for time-of-flight-based
% image recinstruction in 2D.

%%=========================================================================
% GREEN'S INVERSION APPROACH
%==========================================================================
% The reconstructed image of the sound speed using early iterations of a
% time-of-flight-based algorithm will then be used as initial guess for
% novel inversion approaches based on a ray approximation to heterogeneous
% Green's function. Using the ray-based Green's inversion approach, the image
% reconstruction is performed from low to high frequencies. For the Green's
% inversion approaches, the ray tracing is performed only using
% 'Runge-kutta-2nd' (cf. Algorithm 1 in [1].)
% Other ray tracing approaches can be used only for time-of-flight-based
