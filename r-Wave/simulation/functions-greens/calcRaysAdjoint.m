function [ray_position_adjoint, ray_time_adjoint,ray_absorption_adjoint,...
    ...
    ray_direction, ray_spacing, rayspacing_receiver)
...
%                           are traced (not reversed), because the geomterical attenuation
%                           along the rays is not reversible.
%       ray_direction      - a num_emitter x 1 cell array each containing
%                            a num_receiver x 2 matrix contatining the
%                            Cartesian direction of the linked rays
...
%                           are traced (not reversed), because the geomterical attenuation
%                           along the rays is not reversible.
%       ray_direction_adjoint - a num_receiver x 1 cell array each containing
%                            a num_emitter x 2 matrix contatining the
%                            Cartesian direction of the linked rays
%
%
% ABOUT:
...
if isempty(ray_direction{1})
    
    % if the Cartesian direction of the ray is not given,
    % set the associated Boolean false
    do_reverse_direction = false;
else
    
    % if the Cartesian direction of the ray is given,
    % set the associated Boolean true
    do_reverse_direction = true;
end
...
if do_reverse_direction
    ray_direction_x_adjoint = cell(1, num_emitter) ;
    ray_direction_y_adjoint =  cell(1, num_emitter);
end
...
% get the x Cartesian direction of the perturbed rays 
ray_direction_x_adjoint{ind_emitter} = ray_direction{ind_emitter}(1:2:end, :);
% get the y Cartesian direction of the perturbed rays 
ray_direction_y_adjoint{ind_emitter} = ray_direction{ind_emitter}(2:2:end, :);
...
switch auxiliary_method
    ...
    case 'paraxial'
        ...
        if do_reverse_direction
            ...
            % convert the cell to Mat
            ray_direction_x_adjoint = cell2mat(permute(ray_direction_x_adjoint , [1, 3, 2]));
            ray_direction_y_adjoint = cell2mat(permute(ray_direction_y_adjoint , [1, 3, 2]));
            
            % allocate zero matrices for the perturbed direction on the rays
            ray_direction_adjoint = zeros(2*num_emitter, num_maxraypoint, num_receiver);
            
            
            % change the order of the dimensions of matrices to num_emitter x num_raypoints x
            % num_receiver, because for computing the adjoint fields, the emitters and receivers
            % are swapped
            ray_direction_adjoint(1:2:end, :,:) = squeeze(permute(ray_direction_x_adjoint, [3, 2, 1]));
            ray_direction_adjoint(2:2:end, :,:) = squeeze(permute(ray_direction_y_adjoint, [3, 2, 1]));
            
            % convert the three dimensional matrices for the position of points on the auxiliary rays to cell arrays
            % with num_receiver cells corresponding to the excitations for the adjoint field
            ray_direction_adjoint = squeeze(mat2cell(ray_direction_adjoint, 2*num_emitter,...
                num_maxraypoint, ones(num_receiver, 1)));
            

           % ray_direction = cell2mat(permute(ray_direction, [1, 3, 2]));
           % ray_direction = squeeze(permute(ray_direction, [3, 2, 1]));
           % ray_direction_adjoint = squeeze( mat2cell(ray_direction, num_emitter,...
           %     num_maxraypoint, ones(2 * num_receiver, 1)) );
        else
            ray_direction_adjoint = cell(1, num_receiver);
