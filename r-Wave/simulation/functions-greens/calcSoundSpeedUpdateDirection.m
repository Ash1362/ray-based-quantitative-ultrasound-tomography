...
switch para.greens_method
    ...
    case  'analytic'
        ...
        % fractional power plus 1 angular frequencies (y+1)
        omega_frac = omega.^(y+1);
        
        % the spacing in the angular frequencies [rad/s]
        if length(omega) > 1
...
approx_pressure = @(source_pulse, time_delays, geom_attenuation, absorption,...
    ...
    geom_attenuation, absorption, caustic_number, omega, y, para.greens_method, source_mode);
            ...
            if calc_adjointfield
                ...
                for ind_receiver = 1 : num_receiver
                    ...
                    switch para.task
                        ...
                        case {'hessian','hessian-backprojection'}
                            ...
                            if num_rays{ind_emitter}(ind_receiver) < para.max_rays
                                ...
                                res_signal = - 2 * perturbation(nan_grid_binary_adjoint{ind_receiver}) .*...
                                    ...
                                    absorption(nan_grid_binary_adjoint{ind_receiver}) .* omega_frac));
                    ...
                    if any(res_signal)
                        ...
                        switch para.task
                            ...
                            case{'backprojection', 'hessian-backprojection'}
