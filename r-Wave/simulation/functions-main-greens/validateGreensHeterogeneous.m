...
%      'raytogrid_interp' - the approach for interpolation from grid to the sampled
%                           points on the ray (Default: 'Bspline-Denis')
...
%                                                 sampled points along the right auxiliary
%                                                 adjoint ray
%      ray_direction                            - the perturbed direction of the forward rays
...
% Boolean controlling whether the perturbed direction of the ray due to 
% the perturbation to the initial position of the ray is computed or not.
% This approach is not implemented yet, and must be set false.
do_perturb_initial_position = false;
if do_perturb_initial_position
    error(['The approach for peterurbation to the position'...
...
[~, frequency_index] = min(abs(1/(2*pi) * omega - para.frequency_single));
    ...
    ray_args = {'nworker_pool',  para.num_worker_pool, 'interp_method', para.raytogrid_interp ,...
        'raylinking_method', raylinking_method, 'raylinking_initialisation', raylinking_initialisation,...
        'raytogrid_spacing', para.raytogrid_spacing, 'auxiliary_ray', trace_auxiliary_ray,...
        'auxiliary_method', para.auxiliary_method, 'reference_angle', para.angle_perturb_auxiliary,...
        'max_num_points_factor', 2, 'do_perturb_initial_position', false,...
        'smoothing_window_size', para.smoothing_window_size, 'max_iter', para.max_raylinking_iter,...
        'varepsilon', raylinking_threshold, 'angular_frequency_centre',...
        omega(frequency_index), 'absorption_power', medium.alpha_power};
    ...
    [~, ~, ~, ray_position, ray_time, ray_absorption,...
        ...
        adjoint_ray_position_left, adjoint_ray_position_right, ray_direction, ~, ~] =...
    ...
    [~, ~, ~, ray_position_water, ray_time_water, ~, ~,...
        ray_position_left_water, ray_position_right_water, ~, ~, ray_direction_water, ~,~] = ...
...
calc_parameters = @(ray_position, ray_time, ray_absorption,...
    ray_position_left, ray_position_right, ray_perturbed_direction) calcParametersGreens(....
...
% allocate a matrix for the relative dicrepancy between the pressure fieled
% approximated by the Green's function and computed using k-Wave on the
% receivers
% relative_discrepancy_signals = zeros(num_receiver, num_emitter);
...
parfor (ind_emitter = 1: num_emitter, para.num_worker_pool)
    ...
    [parameters_grid_water, parameters_receiver_water, nan_grid_binary_water, ~,...
        ...
        ray_direction_water{ind_emitter});
    ...
    %signal_approximated{ind_emitter} = signal_approximated_emitter;
    %signal_approximated_water{ind_emitter} = signal_approximated_water_emitter;
    %signal_approximated_nonabsorbing{ind_emitter} = signal_approximated_nonabsorbing_emitter;
...
xlabel('receiver index'); ylabel('Amplitude [a.u.]');
legend('Greens, absorbing breast',...
    'Greens, nonabsorbing breast',...
    'Greens, water ',...
