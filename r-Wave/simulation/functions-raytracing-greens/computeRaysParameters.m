function [cartesian_position_endpoint, cell_num_rays, optimal_polar_initial_direction,...
    ...
    polar_direction_auxiliary, adjoint_polar_direction_auxiliary, ...
...
%     polar_direction_auxiliary   - the polar direction of the auxiliary ray
%                                   for the forward field
%     adjoint_polar_direction_auxiliary - the polar direction of the auxiliary
%                                         ray for the adjoint field
...
switch dim
    case 2
        ...
        para.perturbation_position_amplitude = 1e-6;
    case 3
...
if ~(para.auxiliary_ray && strcmp(para.auxiliary_method, 'paraxial'))
    ...
    % if the auxiliary method is not used or if the method for computing
    % auxiliary rays is not paraxial, the perturbation to the initial position
    % is not required
    para.do_perturb_initial_position = false;
end
...
% allocate a cell array for the polar direction of the forward auxiliary rays
polar_direction_auxiliary = cell(1, num_emitter);

% allocate a cell array for the polar direction of the adjoint auxiliary rays
adjoint_polar_direction_auxiliary = cell(1, num_emitter);
...
parfor (ind_emitter = 1 : num_emitter, para.nworker_pool)
    ...
    [optimal_polar_initial_direction{ind_emitter},...
        ...
        polar_direction_auxiliary{ind_emitter}, adjoint_polar_direction_auxiliary{ind_emitter}]...
...
%num_rays = cell2mat(cell_num_rays);
% disp(['the percentage fraction of badlinkings is'  num2str(nnz(num_rays>para.max_iter-1)/nnz(num_rays>1)*100, '%1.5e')])
%clear cell_num_rays
...
%if para.auxiliary_ray
   ...
   % if strcmp(para.auxiliary_method, 'paraxial')
        % give empty variables for the Cartesian position of the right auxiliary rays
        % forward rays
        
        % right
     %   cartesian_position_auxiliary_right = [];
        % right
     %  adjoint_cartesian_position_auxiliary_right = [];
        
        
    %    if ~para.do_perturb_initial_position
            
            % give empty variables for the ploar direction of the auxiliary rays
            % forward rays
    %        polar_direction_auxiliary = [];
            % adjoint rays
          %  adjoint_polar_direction_auxiliary = [];
   %     end
...
% else
    ...
    % give empty variables for the Cartesian position of the auxiliary rays
    % forward rays
    % left
   % cartesian_position_auxiliary_left = [];
    % right
  %  cartesian_position_auxiliary_right = [];
    % adjoint rays
    % left
  %  adjoint_cartesian_position_auxiliary_left = [];
    % right
  %  adjoint_cartesian_position_auxiliary_right = [];
    
    
    % give empty variables for the polar direction of the auxiliary rays
    % forward rays
  %  polar_direction_auxiliary = [];
    % adjoint rays
  %  adjoint_polar_direction_auxiliary = [];
